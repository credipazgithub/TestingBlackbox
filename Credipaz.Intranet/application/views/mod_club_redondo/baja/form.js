...
$("body").off("click", ".btnAccept").on("click", ".btnAccept", function () {
	...
	if (_motivo == "") { alert("Debe ingresar Motivo"); return false; }

	$(".btnAction").hide();
	var _json = {
		"data_function": "baja-club-redondo",
	...
	_AJAX.UiSendExternal(_json).then(function (data) {
		if (data.status == "OK") {
			...
			_FUNCTIONS.onShowInfo("La solicitud de baja ha sido procesada", "SOLICITUD DE BAJA");
		} else {
			throw data;
		}
	}).catch(function (err) {
