...
//ERROR BEHAVIOUR
function logFolders($obj,$values,$method){
    try {
        ...
        $FOLDERS_LOG=$obj->createModel(MOD_FOLDERS,"Folders_log","Folders_log");
        $fields = array(
...
function logFolderItems($obj,$values,$method){
    try {
        if(!isset($values["id_user_active"]) or $obj->table=="folders_log"){throw new Exception(lang('error_9999'),9999);}
        if(!isset($values["id"])){$values["id"]=null;}
        $FOLDER_ITEMS_LOG=$obj->createModel(MOD_FOLDERS,"Folder_items_log","Folder_items_log");
        $fields = array(
            'code' => opensslRandom(8),
...
function unLogFolderItems($obj,$values){
    try {
        if(!isset($values["id_user_active"]) or $obj->table=="folders_log"){throw new Exception(lang('error_9999'),9999);}
        if(!isset($values["id"])){$values["id"]=null;}
        $FOLDER_ITEMS_LOG=$obj->createModel(MOD_FOLDERS,"Folder_items_log","Folder_items_log");
...
//HTML COMBOS
function comboTypeFoldersCode($obj,$get=array("order"=>"description ASC","pagesize"=>-1)){
    $parameters=array(
        "model"=>(MOD_FOLDERS."/Type_folders"),
        "table"=>"type_folders",
        "name"=>"browser_code_type_folder",
        "class"=>"form-control",
        "empty"=>true,
        "id_actual"=>"",
        "id_field"=>"code",
...
function comboTypeFolders($obj,$get=array("order"=>"description ASC","pagesize"=>-1)){
    $parameters=array(
        "model"=>(MOD_FOLDERS."/Type_folders"),
        "table"=>"type_folders",
...
function comboTypeFolderItems($obj,$get=array("order"=>"description ASC","pagesize"=>-1)){
    $parameters=array(
        "model"=>(MOD_FOLDERS."/Type_folder_items"),
...
function comboTypeControlPoints($obj){
    $parameters=array(
        "model"=>(MOD_FOLDERS."/Type_control_points"),
