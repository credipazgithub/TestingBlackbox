...
//ERROR BEHAVIOUR
function logAproval($obj,$values,$method){
    try {
...
function logFolderItemsAproval($obj,$values,$method){
    try {
        if(!isset($values["id_user_active"]) or $obj->table=="folders_log"){throw new Exception(lang('error_9999'),9999);}
        if(!isset($values["id"])){$values["id"]=null;}
...
function unLogFolderItemsAproval($obj,$values){
    try {
        if(!isset($values["id_user_active"]) or $obj->table=="folders_log"){throw new Exception(lang('error_9999'),9999);}
        if(!isset($values["id"])){$values["id"]=null;}
        $FOLDER_ITEMS_LOG=$obj->createModel(MOD_APROVAL,"Folder_items_log","Folder_items_log");
...
//HTML COMBOS
function comboTypeFoldersAproval($obj){
    $parameters=array(
        "model"=>(MOD_APROVAL."/Type_folders"),
        "table"=>"type_folders",
...
function comboTypeFolderItemsAproval($obj){
    $parameters=array(
        "model"=>(MOD_APROVAL."/Type_folder_items"),
...
function comboTypeControlPointsAproval($obj){
    $parameters=array(
        "model"=>(MOD_APROVAL."/Type_control_points"),
...
function comboTypeSectorsAproval($obj){
    $parameters=array(
        "model"=>(MOD_APROVAL."/Type_sectors"),
        "table"=>"type_sectors",
        "name"=>"browser_id_type_sector",
        "class"=>"form-control",
        "empty"=>true,
        "id_actual"=>"",
        "id_field"=>"id",
