var _VIDEOCHAT = {
	_tmrCountdown: 0, //replace
	_tmrVideoActive: 0, //replace
	_limitAlert: (60 * 30), //replace
	_counterVideostatus: 0, //replace
}
...
function initAbmOperatorTasks(_id, _ro, _id_type_task_close, _record) {
	_id_charge_code = _id;
	...
	if (_ro) {
		...
		if (_id_type_task_close != "") {
			...
			if (_record) { $(".btn-record-play").click(); }
		} else {
		...
		$(".btn-videoconferencia").removeClass("d-none");
		if (_record) { $(".btn-record-start").click(); }
	}
...
$("body").off("click", ".btn-record-start").on("click", ".btn-record-start", async () => {
	...
	try {
		...
		_FUNCTIONS.onBlockTelemedicina();
	...
	recorder.onstop = () => {
		...
		try { gumStream.getTracks().forEach(track => track.stop()); } catch (e) { }
		try { gdmStream.getTracks().forEach(track => track.stop()); } catch (e) { }
	};
	recorder.start();
