var _F = {
    ...
    readConfigServers: function (_key) {
        return new Promise(
            function (resolve, reject) {
                fetch("./Recursos/configServers.json?" + _T.UUID())
                    .then(response => {
    ...
    onModalFullScreen: function (_title, _body, _class) {
        ...
        _F.onDestroyModal("#alterFullScreen");
    ...
    onAlert: function (_json) {
        try {
            clearTimeout(_F._timerPushAlert);
            $(".push-alert").remove();
            ...
            _F._timerPushAlert = setTimeout(function () { $(".push-alert").alert('close') }, 7500);
            return true;
        } catch (rex) {
            alert(rex.message);
            return false;
    ...
    onNoInstallApp: function () {
        _F.onDestroyModal("#alterFullScreen", function () {
            setTimeout(function () {
                var _html = "";
                ...
                _html += "   </tr>";
                _html += "</table>";
                _F.onModalAlert("Confirmar acción", _html, "dark", false, function () { });
            }, 500);
        });
    },
    ...
    onModalByIdPost: function (_this) {
        ...
        _API_deprecated.UiGetWebPosts({ "id": _id })
            .then(function (data) {
    ...
    onSelectMyName: function (_this) {
        _F._auth_user_data.viable = _this.attr("data-viable");
        _F._auth_user_data.name = _this.html();
    ...
    onSetInterface: function (user) {
        _F.onGetUserInformation();
        $(".page-sections").html("");
        $(".toggle-sidebar").animate({ opacity: 1 }, 250, function () { });
        $(".displayUserdata").animate({ opacity: 1 }, 250, function () { });
        _F._user_active = user;
        if (_F._logged) {
            var _displayName = _F._auth_user_data.name;
            try { _displayName = _F._auth_user_data.name.split(" ")[0]; } catch (err) {}
            var _pre = "Hola, ";
            if (_displayName == null) { _displayName = user.email.split("@")[0]; }
            $(".displayUserdata").html(_pre + _displayName);
        }
        _F.onToggleVisibilityByStatus();
        /*DEfault access visible point*/
        $(".page-sections").load(_F._default_page, function () {
            $(".app-header").hide();
            $(".app-beneficios").hide();
            $(".app-footer").hide();
            setTimeout(function () {
        ...
        if (_F._logged) {
            ...
            setTimeout(function () {
                if (_F._showPopup) {
                    ...
                    _html += "<img src='img/banner.jpg?" + _T.UUID() + "' style='width:100%;'/>";
                    _html += "</a>";
                    _F.onShowHtmlModal(_html, function () {
                        _F._showPopup = false;
                        setTimeout(function () {
    ...
    onOpenTelemedicina: function () {
        ...
        var _day = d.getDay();
        if (_F._session_data.TelemedicinaDiasAtencion.indexOf(_day.toString()) === -1) { return false; }
        if (_hour < _F._session_data.TelemedicinaHoraDesde || _hour >= _F._session_data.TelemedicinaHoraHasta) { return false; }
        return true;
    },
    ...
    onSeeMap: function (_this) {
        ...
        _F.onDrawMap(".maparea");
    },
    onSeeListado: function (_this) {
        clearInterval(_F._TMR_IMAGES);
    ...
    onPageHeader: function (_target, _title, _img, _back, _map) {
        ...
        _html += "<div class='col-" + _rest + " text-center p-0'><b class='dyn-title'>" + _T.titleCase(_title) + "</b></div>";
        _html += _htmlMap;
    ...
    onEvalLoginStatus: function () {
        ...
        if (_get != null) { _F._auth_user_data = _get; }
        firebase.auth().onAuthStateChanged(function (user) {
            if (user) {
                try {
                    _API_deprecated.UiTestUserValuePWA({ "type": "document", "documentNumber": _F._auth_user_data.dni })
                        .then(function (data) {
                            _F.onAuthenticateMobile(user);
                        }).catch(function (err) {
                            _F.onModalAlert("Error", err.message, "danger");
                        });
                } catch (rex) { }
            } else {
                _DB.Remove("logged");
                _F._logged = false;
                _F._credentialsReady = false;
                _F.onSetInterface(null);
            }
        });
    },
    onGetUserInformation: function () {
        _API_deprecated.UiGetUserInformation(_F._auth_user_data).then(
            function (data) {
                ...
                _F._session_data = JSON.parse(_message);
                _F._id_credipaz = parseInt(_F._session_data.IdCliente);
                _F._nombre = _F._session_data.ApellidoNombre;
                if (_F._session_data.CodigoArea == undefined) { _F._session_data.CodigoArea = ''; }
                if (_F._session_data.Telefono == undefined) { _F._session_data.Telefono = ''; }
                if (_F._session_data.Periodo == undefined) { _F._session_data.Periodo = '201903'; }
                if (_F._session_data.Archivo == undefined) { _F._session_data.Archivo = '0101000231'; }
                if (_F._session_data == null) { _F._session_data = { "IdCliente": 0 }; }
                if (_F._session_data.MontoTelemedicina == undefined) { _F._session_data.MontoTelemedicina = 1000; }
                if (_F._session_data.TeleMedConsultasResto == undefined) { _F._session_data.TeleMedConsultasResto = 0; }
                if (_F._session_data.TeleMedicina == undefined) { _F._session_data.TeleMedicina = "N"; }
                if (_F._session_data.PagoEnLinea == undefined) { _F._session_data.PagoEnLinea = 0; }
                if (_F._session_data.EspecialidadesMedicas == undefined) { _F._session_data.EspecialidadesMedicas = 1; }
                if (_F._session_data.CREDENCIALClub == undefined) { _F._session_data.CREDENCIALClub = 0; }
                if (_F._session_data.CREDENCIALClub == "*") { _F._session_data.CREDENCIALClub = 0; }
                if (_F._session_data.LogoClub == undefined) { _F._session_data.LogoClub = 0; }
                if (_F._session_data.SorteoLibre == undefined) { _F._session_data.SorteoLibre = 0; }
                if (_F._session_data.TelemedicinaHoraDesde == undefined) { _F._session_data.TelemedicinaHoraDesde = 8; }
                if (_F._session_data.TelemedicinaHoraHasta == undefined) { _F._session_data.TelemedicinaHoraHasta = 20; }
                if (_F._session_data.TelemedicinaDiasAtencion == undefined) { _F._session_data.TelemedicinaDiasAtencion = "1234560"; }
                if (_F._session_data.TeleMedicinaRegistrado == undefined) { _F._session_data.TeleMedicinaRegistrado = 1; }

                _F._session_data.TelemedicinaMensajeDias = "Disponible de lunes a sábados de"; 
                _F._session_data.TelemedicinaMensajeCerrado = _F._session_data.TelemedicinaMensajeDias + " " + _F._session_data.TelemedicinaHoraDesde + " a " + _F._session_data.TelemedicinaHoraHasta + "hs."; 
                if (_F._session_data.LegalesConsultasResto == undefined) { _F._session_data.LegalesConsultasResto = 1; }
                if (_F._session_data.LegalesMensajeAtencion == undefined) { _F._session_data.LegalesMensajeAtencion = ""; }
                if (_F._session_data.MontoLegales == undefined) { _F._session_data.MontoLegales = 0; }
                if (_F._session_data.ColorAsociate == undefined) { _F._session_data.ColorAsociate = "#ff24a0"; }
                if (_F._session_data.NombreCredencial == undefined) { _F._session_data.NombreCredencial = ""; }
                if (_F._session_data.PANClub == undefined) { _F._session_data.PANClub = 0; }
                if (_F._session_data.Sube == undefined) { _F._session_data.Sube = 1; }
                if (_F._session_data.resumen == undefined) { _F._session_data.resumen = 1; }
                if (_F._session_data.ClubRedondo == undefined) { _F._session_data.ClubRedondo = 0; }
                if (_F._session_data.MontoCreditoOfrecido == undefined) { _F._session_data.MontoCreditoOfrecido = 0; }
                if (_F._session_data.MontoAdelantoTarjeta == undefined) { _F._session_data.MontoAdelantoTarjeta = 0; }
                if (_F._session_data.OfrecerAdelantoTarjeta == undefined) { _F._session_data.OfrecerAdelantoTarjeta = 0; }
                if (_F._session_data.OfrecerTarjeta == undefined) { _F._session_data.OfrecerTarjeta = 1; }
                if (_F._session_data.OfrecerCredito == undefined) { _F._session_data.OfrecerCredito = 1; }
                if (_F._session_data.text != undefined) { _F.onModalFullScreen("", _F._session_data.text, "warning"); }
                if (_F._session_data.NroTelCredito == undefined) { _F._session_data.NroTelCredito = "08103339009"; }
                if (_F._session_data.NroTelTarjeta == undefined) { _F._session_data.NroTelTarjeta = "08103339009"; }
                if (_F._session_data.NroTelEnfermedadesCriticas == undefined) { _F._session_data.NroTelEnfermedadesCriticas = "08106662582"; }
                if (_F._session_data.NroTeltrasplantes == undefined) { _F._session_data.NroTeltrasplantes = "08106662583"; }
                if (_F._session_data.NroTelCoberturaVida == undefined) { _F._session_data.NroTelCoberturaVida = "08106662584"; }
                if (_F._session_data.NroTelAsisHogar == undefined) { _F._session_data.NroTelAsisHogar = "08003333127"; }
                if (_F._session_data.ModoCredencialSWISS == undefined) { _F._session_data.ModoCredencialSWISS = 0; }


                /* default de valores de botones en la interface */
                if (_F._session_data.botonTelemedicina == undefined) { _F._session_data.botonTelemedicina = 0; }
                if (_F._session_data.botonFarmacias == undefined) { _F._session_data.botonFarmacias = 0; }
                if (_F._session_data.botonUrgencias == undefined) { _F._session_data.botonUrgencias = 0; }
                if (_F._session_data.botonEspecialidades == undefined) { _F._session_data.botonEspecialidades = 0; }
                if (_F._session_data.botonLaboratorios == undefined) { _F._session_data.botonLaboratorios = 0; }

                _F._session_data.botonTelemedicina = parseInt(_F._session_data.botonTelemedicina);
                _F._session_data.botonFarmacias = parseInt(_F._session_data.botonFarmacias);
                _F._session_data.botonUrgencias = parseInt(_F._session_data.botonUrgencias);
                _F._session_data.botonEspecialidades = parseInt(_F._session_data.botonEspecialidades);
                _F._session_data.botonLaboratorios = parseInt(_F._session_data.botonLaboratorios);

                if (parseInt(_F._session_data.IdSocio) != 0) {
                    $(".botonTelemedicina").hide();
                    $(".botonFarmacias").hide();
                    $(".botonUrgencias").hide();
                    $(".botonEspecialidades").hide();
                    $(".botonLaboratorios").hide();
                    if (_F._session_data.botonTelemedicina == 1) { $(".botonTelemedicina").fadeIn("slow"); }
                    if (_F._session_data.botonFarmacias == 1) { $(".botonFarmacias").fadeIn("slow"); }
                    if (_F._session_data.botonUrgencias == 1) { $(".botonUrgencias").fadeIn("slow"); }
                    if (_F._session_data.botonEspecialidades == 1) { $(".botonEspecialidades").fadeIn("slow"); }
                    if (_F._session_data.botonLaboratorios == 1) { $(".botonLaboratorios").fadeIn("slow"); }
                } else {
                ...
                if (parseInt(_F._auth_user_data.dni) == parseInt(_F._TEST_DNI)) {
                    _F._session_data.TeleMedConsultasResto = 10;
                    _F._session_data.MontoTelemedicina = 0;
                }
                console.log(_F._session_data);
    ...
    onChangePage: function (_this) {
        ...
        _F._pagination_actual_value = 1;
        if (_F._actualPage.toUpperCase() == _this.attr("data-href").toUpperCase()) { return false; }
        _F._actualPage = _this.attr("data-href");
        clearInterval(_F._TMR_IMAGES);
        _F.onDestroyModal('#modal_canje');

        /*-------------------------------------------*/
        /* Reset de variables al entar a cada página */
        /*-------------------------------------------*/
        _F._video_active = false;
        /*-------------------------------------------*/
        ...
        if ($(".page-sidebar").css("left") == "0px") {
            ...
            _F.onToggleSidebar(null);
        }
        $(".effect").html($(".page-header").html() + $(".page-sections").html());
        $(".effect").css({ "left": "0px", "opacity": 1 }).show();
        if (_this.attr("data-id") != undefined) { _F.id_category = _this.attr("data-id"); }
        $(".searcher").attr("data-category", _F.id_category);

        var ms = Date.now();
        $(".page-sections").load((_F._server + _F._actualPage + "?" + ms),
            function () {
                _F.onToggleVisibilityByStatus();
    ...
    onAuthenticateMobile: function (user) {
        _API_deprecated.UiAuthenticateMobile({
            "id": _F._auth_user_data.id,
            "field": "username",
            "value": user.email,
            "email": user.email,
            "dni": _F._auth_user_data.dni,
            "sex": _F._auth_user_data.sex,
            "area": _F._auth_user_data.area,
            "phone": _F._auth_user_data.phone,
            "viable": _F._auth_user_data.viable,
            "name": _F._auth_user_data.name,
            "password": _F._auth_user_data.password,
            "IdSolicitud": 0
        }).then(function (data) {
            try {
                console.log("data->");
                ...
                if (data.userdata != null) {
                    _F._auth_user_data.id_club_redondo = 0;
                    if (data.clubredondo != undefined) { _F._auth_user_data.id_club_redondo = data.clubredondo.ClubRedondo; }
                    _F._auth_user_data.id = data.id;
                    _F._auth_user_data.dni = data.userdata.documentNumber;
                    if (_F._auth_user_data.dni.includes("@")) { _F._auth_user_data.dni = _F._auth_user_data.dni.split("@")[0]; }
                    _F.urlPago = ("https://intranet.credipaz.com/linkDirect/pagos-fiserv-pwacr/" + _T.utf8_to_b64(_F._auth_user_data.dni));
                    $(".linkPago").attr("href", _F.urlPago);
                    _F._auth_user_data.sex = data.userdata.documentSex;
                    _F._auth_user_data.area = data.userdata.documentArea;
                    _F._auth_user_data.phone = data.userdata.documentPhone;
                    _F._auth_user_data.viable = data.userdata.viable;
                    _F._auth_user_data.name = data.userdata.documentName;

                    _HTTPREQUEST_deprecated._token_authentication = data.token_authentication;
                    _HTTPREQUEST_deprecated._id_user_active = _F._auth_user_data.id;
                    _HTTPREQUEST_deprecated._id_club_redondo = _F._auth_user_data.id_club_redondo;
                    _DB.Set("logged", _F._auth_user_data);
                    _F._logged = true;
                    _F.onSetInterface(user);
                } else {
                    throw new Error();
                }
            } catch (err) {
    ...
    onActionLogin: function (_this) {
        ...
        if (!$("#chkTerminos").prop("checked") && _step != "firstforced" && _step != "lastforced") {
            _F.onModalAlert("Alerta", "Debe aceptar los términos y condiciones de uso.", "info");
            return false;
        }
        var _dni = $(".dni").val();
        var _password = $(".password").val();
        var _email = (_dni + _HTTPREQUEST_deprecated._sufixEmail);
        switch (_step) {
            case "firstforced":
            case "first":
                _API_deprecated.UiTestUserValuePWA({ "type": "document", "documentNumber": _dni })
                    .then(function (data) {
            ...
            case "lastforced":
            case "last":
                _F._auth_user_data.id = 0;
                _F._auth_user_data.email = _email;
                _F._auth_user_data.sex = $(".sex").val();
                _F._auth_user_data.area = $(".area").val();
                _F._auth_user_data.phone = $(".phone").val();
                _F._auth_user_data.password = $(".password").val();
                firebase.auth().signInWithEmailAndPassword(_email, "123456")
                    .then(function (data) {
                        _DB.Set("logged", _F._auth_user_data);
                        _F.onAuthenticateMobile(_F._auth_user_data);
                    })
                    .catch(function (error) {
                        firebase.auth().createUserWithEmailAndPassword(_email, "123456")
                            ...
                            .catch(function (error) {
                                _F.onModalAlert("Alerta", "Usuario y/o contraseña erróneas.  Reintente.", "danger");
    ...
    onActionLogout: function (_this) {
        ...
        firebase.auth().signOut().then(function () { }, function (error) {
            _F.onModalAlert("Error", JSON.stringify(error), "danger");
        });
        _DB.Remove("logged");
        window.location = "/";
    },
    onActionExit: function (_this) {
        if (!$("#chkRecordarme").prop("checked")) { _F.onActionLogout(null); }
        window.close();
    },
    onActionForget: function (_this) {
        if (!_T.validate(".validateForget")) {
            _F.onModalAlert("Datos faltantes", "Complete los datos requeridos", "warning");
            return false;
        }
        var _email = ($("#dni").val() + _HTTPREQUEST_deprecated._sufixEmail);
        var _json = { "id": 0, "email": _email, "module": "mod_backend", "table": "users", "model": "users" };
        _API_deprecated.UiDelete(_json).then(function (datajson) {
            if (datajson.status == "OK") {
                firebase.auth().signInWithEmailAndPassword(_email, "123456")
                    .then(function (data) {
                        firebase.auth().currentUser.delete();
                    }).catch(function (error) { });
                _F.onModalAlert("Información", "Debe realizar el proceso de registro nuevamente.", "info");
                _F.onActionLogout(null);
            }
        });
    },
        ...
        _API_deprecated.UiRecetasTelemedicina({ "id_charge_code": _F._id_charge_code, "request_mode": "actual" })
            .then(function (data) {
                if (data.status == "OK") {
                    ...
                    var _rec = _T.utf8_to_b64(JSON.stringify(data.data));
                    $(_target_recetas).attr("data-rec", _rec);
                    _html = "<ul class='list-group'>";
                    $.each(data.data, function (i, obj) {
                        var _str = _T.utf8_to_b64(JSON.stringify(obj));
    ...
    onViewDirectTelemedicina: function (_this) {
        try {
            _F.onDestroyModal('#telemedicinaModalView');
            var _data = _this.attr("data-item");
            if (_data.indexOf("<div") > -1) {
                _data = _data.split(",")[0];
                _data = _data.split("<")[0];
            }
            var _json = JSON.parse(_T.b64_to_utf8(_data));
            var _raw_data = JSON.parse(_json.raw_data);
            /*Si es enviado por el usuario, se marca como verificado, leido!*/
            _API_deprecated.UiViewMessagesTelemedicina({ "id": _json.id }).then(function (data) { });
            ...
            _F.onModalAlert("Error", rex.message, "danger");
            return false;
        }
    },
    onViewDirectTelemedicinaPDF: function (_this) {
        try {
            _F.onDestroyModal('#telemedicinaModalViewPDF');
            var _data = _this.attr("data-item");
            if (_data.indexOf("<div") > -1) {
                _data = _data.split(",")[0];
                _data = _data.split("<")[0];
    ...
    onMisComprobantes: function (_this) {
        _API_deprecated.UiComprobantesTelemedicina({})
            .then(function (data) {
                if (data.status == "OK") {
                    $(".mis-recibos").html(_F.onDrawRecibos(data.recibos, "recibos", "receipt_long"));
                    $(".mis-facturas").html(_F.onDrawFacturas(data.facturas, "facturas", "receipt"));
                }
            });
    },
    ...
    onDrawRecibos: function (data, _type, _icon) {
        ...
        $.each(data.data, function (i, obj) {
            ...
            _html += "<tr>";
            _html += "   <td width='80%'>";
            _html += "      <a href='#' class='btn btn-sm btn-raised btn-block btn-primary btn-see-comprobante' data-mode='" + _type + "' data-item='" + _str + "'>" + _T.toDDMMYY(obj.created, "-") + "</a>";
            _html += "   </td>";
            _html += "</tr>";
        });
        _html += "</table>";
    ...
    onSeeComprobante: function (_this) {
        ...
        try {
            _F.onDestroyModal('#telemedicinaComprobante');
            var _data = _this.attr("data-item");
            if (_data.indexOf("<div") > -1) {
                _data = _data.split(",")[0];
                _data = _data.split("<")[0];
            }
            var _raw_data = JSON.parse(_T.b64_to_utf8(_data));
            var _title = "";
            var _numero = "";
            var _body = "";
            switch (_type) {
                case "recibos":
                    _title = "RECIBO";
                    _body = _T.b64_to_utf8(_raw_data.base64_2);
                    break;
                case "facturas":
                    _title = "FACTURA";
                    var _ptoVta = _T.LPAD(_raw_data.Prefijo, '0', 5);
                    var _nroCmp = _T.LPAD(_raw_data.NroComprobante, '0', 8);
                    ...
                    _body += "                  <td valign='bottom'>" + _T.toDDMMYY(_raw_data.Fecha, "-") + "</td>";
            ...
            switch (_type) {
                case "facturas":
                    var _param = _T.utf8_to_b64(JSON.stringify(_raw_data.QR));
    ...
    onGetPDF: function (_this) {
        ...
        _T.HtmlToPdfFile(_type, ("#" + _docArea), _fileName, _title, _text, function (data) {
            _F.onDestroyModal('#telemedicinaModalView');
            _F.onDestroyModal('#telemedicinaModalViewPDF');
            _F.onDestroyModal('#telemedicinaComprobante');
            if (!data) { _F.onModalAlert("Alerta", "Su dispositivo no permite compartir archivos.", "info"); }
        });
    },
    onVerCupon: function (_this, _canjea) {
        var _html = "";
        _F._last_image = $("." + _this.attr("data-img-ref")).attr("src");
        ...
        if (_F._last_image != "" && _F._last_image != undefined) {
            _html += "<table align='center' style='width:80%;color:grey;'>";
            _html += "   <tr>";
            _html += "      <td valign='middle' align='center'>";
            _html += "         <img class='img-beneficio' src='" + _F._last_image + "' style='width:80%;'/>";
            _html += "      </td>";
            _html += "   </tr>";
            _html += "</table>";
        } else {
        ...
        _html += "   <i class='material-icons' style='color:grey;font-size:40px;'>chevron_left</i>";
        _html += "</button><br/>";
        if (_F.showLinkAutorizaciones) { _html += "<center><a href='mailto:autorizaciones@gerdanna.com' class='btn btn-success btn-sm' target='_blank'>Autorización de estudios</a></venter>"; }
    ...
    onIsNotSocio: function (_msg) {
        if (!_F._logged) {
            _F.onModalAlert("Alerta", "Por favor, inicie sesión para solicitar servicios", "warning");
            setTimeout(function () { $(".btnLogin").click(); }, 250);
            return false;
        }
        _F.onModalAlert("Alerta", _msg, "info");
        window.open("https://mediya.com.ar/asociate/", "_blank");
    },
    onSearchFromBar: function (_this) {
        _F._pagination_actual_value = 1;
        _F.id_category = _this.attr("data-category");
        _F.onGoForBeneficios("buscar", _F.id_category, _this.val(), 0);
    },
    onGoForBeneficios: function (_mode_categoria, _id_type_category, _coords, _near) {
        ...
        _F.mode_categoria = _mode_categoria;
        _F.type_categoria = _id_type_category;
        ...
        setTimeout(function () {
            ...
            _F._lastjson_getCupons = {
                "mode_categoria": _F.mode_categoria,
                "type_categoria": _F.type_categoria,
                "search": "",
                "coords": _coords,
                "near": _near,
                "lat": _lat,
                "lng": _lng,
                "id_type_vademecum": -1,
                "page": _F._pagination_actual_value
            };
            _API_deprecated.UiGetCupons(_F._lastjson_getCupons).then(function (data) {
                _F.onBuildBeneficiosResults(data);
            }).catch(function (err) {
                _F.onModalAlert("Error", JSON.stringify(err), "danger");
            });
        }, 250);
    ...
    onNextResultPage: function (_this) {
        _F._pagination_actual_value += 1;
        _F._lastjson_getCupons.page = _F._pagination_actual_value;
    ...
    onBuildBeneficiosResults(data) {
        if (data.status == "OK") {
            ...
            switch (parseInt(_F.type_categoria)) {
                case -125://Farmacias
            ...
            $.each(data.message.data, function (i, obj) { try { _html += _F.onBuildBeneficioItem(obj, _canjea, false); } catch (err) { } });
            ...
            clearInterval(_F._TMR_IMAGES);
            _F._TMR_IMAGES = setInterval(function () { _F.onLoadImagesInScreen(".img-loader"); }, 500);
            if (_F._pagination_actual_value == 1) {
    ...
    onBuildBeneficioItem: function (obj, _canjea, _verCupon) {
        try {
            ...
            var _phone = "";
            var _distance = "";
            var _amount = _T.stripHtml(obj.amount);
            ...
            if (_verCupon) {
                ...
                _amount = _T.stripHtml(obj.des_amount);
                _canjeado = ("Canjeado el " + obj.date_canje);
                _classForCanje = "btn-vercanje";
            }
            if (!_canjea) { _image = "./img/Comun/item_" + _F.type_categoria + ".png"; }
            if (_address != "") {
            ...
            _html = "<div class='row no-gutters py-1 " + _classForCanje + " item-cupon item-cupon-" + obj.id + "' data-img-ref='img-" + obj.id + "' data-record='" + _T.utf8_to_b64(JSON.stringify(obj)) + "'>";
    ...
    onLoadImagesInScreen: function (_selector) {
        $(_selector).each(function () {
            ...
            if (obj.attr("src") == "./img/Comun/placeholder.png") {
                if (_T.isElementVisible(obj[0])) {
                    var _data = obj.attr("data-load").split(":");
                    var _json = { "id": _data[1], "type": _data[0] };
                    _API_deprecated.UiGetImage(_json).then(function (data) {
                        if (data.status == "OK") {
    ...
    onSearchMap: function (_categoria) {
        ...
        setTimeout(function () {
            ...
            if (_search == "") {
                _F.onModalAlert("Alerta", "¡Tenés que buscar por provincia y/o ciudad!", "warning");
                return false;
            }
            var _json = {
                "mode_categoria": _F.mode_categoria,
            ...
            _API_deprecated.UiGetCupons(_json).then(function (data) {
                _F.onBuildMapResults(data);
            }).catch(function (err) {
                _F.onModalAlert("Error", JSON.stringify(err), "danger");
            });
        }, 250);
    ...
    onBuildMapResults(data) {
        ...
        if (data.status == "OK") {
            ...
            if (_dirty) {
                ...
                if (parseInt(_F.type_categoria) >= 323) { _msg = "Llama al <a href='tel:08103336488'>0810 333 6488</a>"; }
                _F.onModalAlert("Alerta", _msg, "warning");
            }
    ...
    onBuildItemMap: function (obj) {
        ...
        try {
            ...
            _html = "<div style='cursor:pointer;' class='col-12 no-gutters text-left item-mapa' data-record='" + _T.utf8_to_b64(JSON.stringify(obj)) + "'>";
    ...
    onSelectItemMapa: function (_this) {
        ...
        if (obj.lat != "0" && obj.lng != "0") {
            ...
            var _icon = { url: _F._map_icon, scaledSize: new google.maps.Size(_size.width, _size.height) };
            ...
            marker.addListener('click', function (event) {
                ...
                const infowindow = new google.maps.InfoWindow({
                    content: "<div style='width:220px;'>" + _F.onBuildItemMap(obj) + "</div>"
                });
    ...
    onToggleCobertura: function (_this) {
        ...
        if (_this.attr("data-status") == "off") {
            ...
            switch (_this.attr("data-cobertura")) {
                case "salud":
                    _API_deprecated.UiGetWebPosts({ "id": 101 })
                        .then(function (data) {
                    ...
                    break;
                case "vida":
                    _API_deprecated.UiGetWebPosts({ "id": 103 })
                        .then(function (data) {
                            $(".mensaje-cobertura").html(data.data[0].body_post).removeClass("d-none");
    ...
    drawCredentialSwiss: function (datajson) {
        ...
        $.each(datajson.data, function (i, item) {
            _F.drawCredential("canvasSwiss", "./img/Credenciales/swiss.jpg", "swiss", item).then(function (data) {
                _F._credencialSwiss = data;
                if (item.NroCredencial != "") { _F.drawItemCredential(_F._credencialSwiss, item, "swiss"); }
            });
            $(".area-swiss").removeClass("d-none");
            $(".title-credencial").show();
            _F._credentialsReady = true;
        });
    ...
    drawCredentialGerdanna: function (datajson) {
        ...
        $.each(datajson.data, function (i, item) {
            _F.drawCredential("canvasGerdanna", "./img/Credenciales/gerdanna.jpg", "gerdanna", item).then(function (data) {
                _F._credencialGerdanna = data;
                if (item.NroCredencial != "") { _F.drawItemCredential(_F._credencialGerdanna, item, "gerdanna"); }
            });
    ...
    onEvalCallStatus: function (_json) {
        ...
        _API_deprecated.UiStatusTelemedicina(_params)
            .then(function (data) {
                console.log(data);
                if (!_F._joined) {
                ...
                if (data.status == "OK" && data.paycode != "0" && data.paycode != "") {
                    ...
                    _F._id_charge_code = parseInt(data.paycode);
...
if (window.location.hostname != "localhost") { _F._server = "./"; }
