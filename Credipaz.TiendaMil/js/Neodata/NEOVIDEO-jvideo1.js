var _NEOVIDEO = {
			...
			function (resolve, reject) {
				try {
					...
					var _options = {
						...
						//onload: function () { $('[name^="jitsiConferenceFrame"]').on('load', function () { _NEOVIDEO._API_deprecated.dispose(); }); },
						userInfo: _userInfo,
						configOverwrite: _NEOVIDEO._CONFIG_OVERWRITE,
						interfaceConfigOverwrite: _NEOVIDEO._INTERFACE_CONFIG_OVERWRITE
					}
					if (_NEOVIDEO._API != null) { _NEOVIDEO._API_deprecated.dispose(); };
					_NEOVIDEO._API = new JitsiMeetExternalAPI(_json.domain, _options);
					_NEOVIDEO._API_deprecated.executeCommand('subject', ' ');
					setTimeout(function () {
						_NEOVIDEO._API_deprecated.addEventListeners(
							{
								...
								videoConferenceJoined: function (data) { if ($.isFunction(_NEOVIDEO.onVideoConferenceJoined)) { _NEOVIDEO.onVideoConferenceJoined(data); } },
	...
	onTurnOffVideo: function (_json) {
		return new Promise(
			function (resolve, reject) {
				try {
					_NEOVIDEO._API_deprecated.executeCommand('hangup');
					resolve(true);
				} catch (err) {
					reject(err);
				}
